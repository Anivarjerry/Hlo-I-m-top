...
export const Dashboard: React.FC<DashboardProps> = ({ credentials, role, userName, onLogout }) => {
  ...
  // Safety Timeout for APK Initial Load
  useEffect(() => {
    const timer = setTimeout(() => {
        if (initialLoading) {
            console.warn("Dashboard load timeout triggered.");
  ...
  useEffect(() => {
    ...
    try {
        ...
        } else if (window.history.state.view === 'profile') {
  ...
  const handleStartTrip = (e: React.MouseEvent) => {
    ...
    navigator.geolocation.getCurrentPosition(async (initialPos) => {
        ...
        watchId.current = navigator.geolocation.watchPosition(async (pos) => {
          ...
          alert("GPS Permission Denied. Please enable location for the app."); 
      }, options);
  };
  ...
  // APK "BLACK SCREEN" FIX: If initialLoading is true, render a visible spinner
  if (initialLoading) {
      return (
        <div className="fixed inset-0 flex flex-col items-center justify-center bg-[#F8FAFC] dark:bg-dark-950 z-[999]">
             <div className="w-20 h-20 bg-brand-500/10 rounded-3xl flex items-center justify-center text-brand-500 mb-6 animate-pulse">
                <Sparkles size={40} />
             </div>
             <Loader2 className="animate-spin text-brand-500 mb-4" size={32} />
             <p className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] animate-pulse">Syncing Services...</p>
             <div className="mt-10 px-8 text-center max-w-xs">
                <p className="text-[10px] font-bold text-slate-400 leading-relaxed italic opacity-60">"Initial sync may take a few seconds in APK mode depending on your connection."</p>
             </div>
        </div>
      );
  }
  ...
  if (hasError && !data) {
      return (
        <div className="fixed inset-0 flex flex-col items-center justify-center bg-[#F8FAFC] dark:bg-dark-950 z-[999] px-10 text-center">
             <div className="w-20 h-20 bg-rose-500/10 rounded-3xl flex items-center justify-center text-rose-500 mb-6">
                <WifiOff size={40} />
             </div>
             <h2 className="text-xl font-black text-slate-800 dark:text-white uppercase mb-2">Sync Failed</h2>
             <p className="text-xs font-bold text-slate-500 mb-8">Could not connect to VidyaSetu cloud servers. Please check your data connection.</p>
             <button onClick={() => { setInitialLoading(true); fetchGenericData(); }} className="w-full py-5 rounded-[2rem] bg-brand-500 text-white font-black uppercase text-xs tracking-widest shadow-xl">Try Re-Sync</button>
             <button onClick={onLogout} className="mt-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Logout & Try Again</button>
        </div>
      );
  }

  return (
    <div className="fixed inset-0 h-full w-full bg-[#F8FAFC] dark:bg-dark-950 flex flex-col overflow-hidden transition-colors">
            ...
            <>
                <div className="w-full px-4 pt-3 pb-0.5 z-[40] flex-shrink-0">
                    <div className="max-w-4xl mx-auto w-full">
                        {(!data) ? <SkeletonSchoolCard /> : <SchoolInfoCard schoolName={data?.school_name || ''} schoolCode={data?.school_code || ''} onClick={handleSchoolCardClick} />}
        ...
        <div className="flex flex-col h-[70vh]">
          {!listCategory ? (
             <div className="space-y-6 overflow-y-auto no-scrollbar pb-4 flex-1 relative">
                <button onClick={handleSyncSchoolSummary} disabled={schoolSummaryRefreshing} className={`absolute top-0 right-0 p-2.5 rounded-full bg-slate-100 dark:bg-white/10 text-slate-500 z-20 active:scale-90 transition-all ${schoolSummaryRefreshing ? 'animate-spin text-brand-500' : 'hover:text-brand-500'}`}><RefreshCw size={18} /></button>
                <div className="p-6 rounded-[2.5rem] bg-slate-900 dark:bg-slate-800 text-white shadow-xl relative overflow-hidden mt-2"><div className="relative z-10 text-center"><div className="w-16 h-16 bg-white/10 rounded-2xl flex items-center justify-center mx-auto mb-3 backdrop-blur-md border border-white/20"><SchoolIcon size={32} /></div><h2 className="text-xl font-black uppercase tracking-tight leading-tight">{schoolSummary?.school_name || (loadingSchoolSummary ? 'Loading...' : 'Unknown')}</h2><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Code: {schoolSummary?.school_code || '---'}</p></div></div>
                ...
                <div className="grid grid-cols-2 gap-3"><div onClick={() => handleCategoryClick('teachers')} className="p-4 rounded-[2rem] bg-white dark:bg-white/5 border border-slate-100 dark:border-white/10 shadow-sm cursor-pointer active:scale-95 transition-all"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Teaching Staff</p><div className="flex items-end gap-2"><span className="text-3xl font-black text-slate-800 dark:text-white">{schoolSummary?.total_teachers || 0}</span><ChevronRight size={16} className="text-slate-300 mb-1" /></div></div><div onClick={() => handleCategoryClick('drivers')} className="p-4 rounded-[2rem] bg-white dark:bg-white/5 border border-slate-100 dark:border-white/10 shadow-sm cursor-pointer active:scale-95 transition-all"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">Transport</p><div className="flex items-end gap-2"><span className="text-3xl font-black text-slate-800 dark:text-white">{schoolSummary?.total_drivers || 0}</span><ChevronRight size={16} className="text-slate-300 mb-1" /></div></div></div>
             </div>
          ) : (
             <div className="flex flex-col h-full premium-subview-enter">
                ...
                <div className="flex-1 overflow-y-auto pr-1 space-y-3 no-scrollbar">{loadingUserList ? (<div className="text-center py-10"><Loader2 className="animate-spin mx-auto text-brand-500" /></div>) : categoryUserList.length === 0 ? (<div className="text-center py-10 opacity-30 text-[10px] font-black uppercase tracking-widest">No records found</div>) : (categoryUserList.map((user, i) => (<div key={i} className="p-4 bg-white dark:bg-white/5 rounded-[1.5rem] border border-slate-100 dark:border-white/5 flex items-center gap-3 shadow-sm"><div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-white/10 flex items-center justify-center font-black text-slate-500 dark:text-slate-300">{user.name.charAt(0)}</div><div><p className="font-black text-sm text-slate-800 dark:text-white uppercase truncate">{user.name}</p><p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{user.mobile}</p></div></div>)))}</div>
             </div>
          )}
        </div>
      </Modal>
